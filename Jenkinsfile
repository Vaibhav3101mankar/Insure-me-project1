node{
    ...
    stage('git code checkout'){
        ...
        git "https://github.com/Vaibhav3101mankar/Insure-me-project1.git"
    }
     catch(Exception e){
            ...
            ${BUILD_URL}''', subject: 'Job ${JOB_NAME} ${BUILD_NUMBER} is failed', to: 'vaibhav3101mankar@gmail.com'
        }
    }
    ...
    stage('publish test reports'){
     ...
     stage('Containerize the application'){
        ...
        sh "${dockerCMD} build -t vaibhav3101mankar/insure-me:${tagName} ."
    }
     stage('Pushing it ot the DockerHub'){
        echo 'Pushing the docker image to DockerHub'
        withCredentials([string(credentialsId: 'dock-password', variable: 'dockerHubPassword')]) {
        sh " ${dockerCMD} login -u vaibhav3101mankar -p ${dockerHubPassword}"
        sh " ${dockerCMD} push vaibhav3101mankar/insure-me:${tagName}"
    }
